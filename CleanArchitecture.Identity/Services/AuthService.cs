...
public class AuthService(UserManager<ApplicationUser> userManager, SignInManager<ApplicationUser> signInManager, JwtSettings jwtSettings) : IAuthService
{
    ...
    {
        var user = await _userManager.FindByEmailAsync(request.Email) ?? throw new NotFoundException("user", request.Email);
    ...
    public async Task<RegistrationResponse> Register(RegistrationRequest request)
    {
        ...
        if (existingUser != null) throw new BadRequestException($"Username {request.Username} already taken by another account");

        var existingEmail = await _userManager.FindByEmailAsync(request.Email);
        if (existingEmail != null) throw new BadRequestException($"Email already register. Please sign in");
